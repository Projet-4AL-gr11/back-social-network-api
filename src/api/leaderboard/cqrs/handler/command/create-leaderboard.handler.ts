...
export class CreateLeaderboardHandler implements ICommandHandler<CreateLeaderboardCommand> {
    ...
    async execute(command: CreateLeaderboardCommand): Promise<Leaderboard> {
        try {
            ...
            if (currentDate > command.exercise.endingDate) {
                ...
                const err = await validate(leaderboard, {validationError: {target: false}});
                if (err.length > 0) {
                    throw err;
                }
                const newLeaderboard = await this.leaderboardRepository.save(leaderboard);
                this.eventBus.publish(new CreateLeaderboardEvent(newLeaderboard.id, leaderboard.user.id, leaderboard.exercise.id, leaderboard.language.name));

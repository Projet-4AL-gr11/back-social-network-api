...
export class UpdateEventRankingHandler implements ICommandHandler<UpdateEventRankingCommand> {
    ...
    async execute(command: UpdateEventRankingCommand): Promise<void> {
        try {
            for (let a = 0; a < command.participants.length; a++){
                ...
                for (let i =0; i < command.allLeaderboard.size; i++) {
                    for (let j = 0; j < command.allLeaderboard.get(i).length; j++) {
                        if (command.allLeaderboard.get(i)[j].user.id === command.participants[a].id) {
                            score = command.participants.length - command.allLeaderboard.get(i)[j].ranking
                        }
                    }
                }
                const eventRanking: EventRanking = await this.eventRankingRepository.create({
                    user: command.participants[a],
                    event: command.event,
                    score: score,
                })

                const err = await validate(eventRanking, {validationError: {target: false}});
                if (err.length > 0) {
                    throw err;
                }
            ...
            this.eventBus.publish(new ErrorsEvent('UpdateEventRankingHandler', error));
            throw error;
        }
    }
}
